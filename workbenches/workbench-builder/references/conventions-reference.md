...
The most important conventions for building a new workbench:

- **Five-layer routing**: AGENTS.md (Layer 0) -> CONTEXT.md (Layer 1) -> Stage CONTEXT.md (Layer 2) -> Reference material (Layer 3) -> Working artifacts (Layer 4). Layer 3 is persistent context (design systems, voice rules, conventions). Layer 4 is per-run context (previous stage outputs, source material). See "Five-Layer Routing Architecture" in `/_core/CONVENTIONS.md`.
...
- **Shared constants**: Code-producing workbenches define shared constant files (colors, fonts, timing) that all outputs import from. See "Pattern 15: Shared Constants" in `/_core/CONVENTIONS.md`.
