# Example Workbench: Script-to-Animation

A condensed summary of the script-to-animation workbench. Use this as a reference when building new workbenches to understand what a completed ICM workbench looks like.
...
Skills are copied from local installations or cloned from GitHub during workbench scaffolding. They replace custom reference docs when the skill covers the same ground.
...
Each stage has its own `references/` folder with workbench-specific files:
...
**Skills vs. references:** The remotion-best-practices skill handles all Remotion API knowledge (interpolate, spring, Sequence, etc.). The workbench-specific build-conventions.md handles folder structure, beat patterns, and shared constants integration that are unique to this workflow.
...
- **Stage removal:** If the user does not need the build stage, the entire `stages/03-build/` folder is removed during onboarding. The workbench CONTEXT.md has a `{{?BUILD_STAGE}}...{{/BUILD_STAGE}}` block around the build stage routing entry.
