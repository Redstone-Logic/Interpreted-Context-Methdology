...
Build the onboarding questionnaire that hydrates the new workbench's placeholder variables.
...
2. Scan all markdown files in the scaffolded workbench for `{{PLACEHOLDER}}` patterns. Build a complete list.
...
9. If the workbench has voice/style rules with derived fields, add a two-pass process: after the agent populates rules from answers, it presents them to the user for review before finalizing. This catches misinterpretations.
...
|-------|---------------|
| Placeholder coverage | Every system-level placeholder in the workbench has a corresponding question |
...
|----------|----------|--------|
| Questionnaire | `output/questionnaire.md` | Flat, all-at-once questionnaire for the new workbench's setup/ folder |
