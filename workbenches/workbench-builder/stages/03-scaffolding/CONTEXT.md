...
Generate the complete workbench folder structure, CONTEXT.md files, and placeholder reference files.
...
| Template | `/_core/templates/workbench-agents-template.md` | Full file | Template for the workbench AGENTS.md (includes What to Load section) |
| Template | `/_core/templates/workbench-context-template.md` | Full file | Template for the workbench CONTEXT.md |
...
2. Create the workbench folder structure:
   - Root: AGENTS.md, CONTEXT.md, setup/
...
4. Create the workbench AGENTS.md using the template: folder map, triggers, routing table, and What to Load section mapping each task to its minimal file set
5. Create the workbench CONTEXT.md using the template: task routing table, shared resources
6. Create placeholder reference files for each stage with `{{PLACEHOLDER}}` variables for user-specific content
7. For content-producing workbenches, create a value framework reference file (see Pattern 13)
8. For code-producing workbenches, create a shared constants file or pattern (see Pattern 15)
9. Create the context folder (brand-vault equivalent) with placeholder files. If the workbench produces voice/style content, structure voice rules with Hard Constraints, Sentence Rules, and Pacing sections (not a single description placeholder)
...
|----------|----------|--------|
| Generated workbench | `output/` | Complete folder structure with all files. Ready for questionnaire design. |
